...
importfile() {
  ...
  vector() {
    ...
    cmd="vector::publish -i '$filepath' -o '$outputlayername' -l '$login' -p '$passwd' -h '$dbhost'
  ...
  raster() {
    ...
    cmd="raster::publish -i '$filepath' -o '$outputlayername' -l '$login' -p '$passwd' -h '$dbhost'
...
main() {
  ...
  local host dbhost login passwd workspace datastore pg_datastore db dbuser logs
  source "$paramfile"
